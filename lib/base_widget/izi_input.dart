...
class _IZIInputState extends State<IZIInput> {
  ...
  void initState() {
    ...
    if (widget.type == IZIInputType.INCREMENT ||
      ...
      numberEditingController = MoneyMaskedTextController(
        initialValue:
            IZINumber.parseDouble(widget.initValue ?? widget.min.toString()),
        precision: 0,
      );
    } else {
      numberEditingController = MoneyMaskedTextController(
  ...
  void setValue(dynamic newValue) {
    if (!IZIValidate.nullOrEmpty(newValue) &&
            widget.type == IZIInputType.NUMBER ||
        !IZIValidate.nullOrEmpty(newValue) &&
            widget.type == IZIInputType.PRICE ||
        !IZIValidate.nullOrEmpty(newValue) &&
  ...
  void datePicker(IZIPickerDate pickerType) {
    ...
    if (pickerType == IZIPickerDate.CUPERTINO) {
                  ...
                  onDateTimeChanged: (value) {
                    if (widget.isTimePicker!) {
                      if (widget.cupertinoDatePickerMode != null &&
                        ...
                        // final String date = IZIDate.formatDate(value, format: 'MM/dd/yyyy');
                        getController(widget.type).text =
                            IZIDate.formatDate(value, format: 'HH:mm'); //date;
                        if (!IZIValidate.nullOrEmpty(widget.onChanged)) {
                          widget.onChanged!(IZIDate.formatDate(value,
                              format: 'MM/dd/yyyy HH:mm'));
                        }
                      ...
                      final String date =
                          IZIDate.formatDate(value, format: 'dd/MM/yyyy');
                      getController(widget.type).text = date;
                      if (!IZIValidate.nullOrEmpty(widget.onChanged)) {
                        widget.onChanged!(
                            IZIDate.formatDate(value, format: 'dd/MM/yyyy'));
                  ...
                  if (IZIValidate.nullOrEmpty(value)) {
                    ...
                    if (widget.isTimePicker!) {
                      if (widget.cupertinoDatePickerMode != null &&
                        ...
                        // getController(widget.type).text = date;
                        // if (!IZIValidate.nullOrEmpty(widget.onChanged)) {
                        //   widget.onChanged!(IZIDate.formatDate(now, format: 'MM/dd/yyyy'));
                        // }
                        getController(widget.type).text =
      ...
      showDatePicker(
        context: context,
        initialDate:
            widget.initDate ?? widget.minimumDate ?? IZITimeZone.dateTimeNow(),
        firstDate: widget.minimumDate ??
            IZITimeZone.dateTimeNow().subtract(const Duration(days: 18250)),
        lastDate: widget.maximumDate ??
            IZITimeZone.dateTimeNow().add(const Duration(days: 18250)),
        ...
        (value) {
          if (!IZIValidate.nullOrEmpty(value)) {
            ...
            if (!IZIValidate.nullOrEmpty(widget.onChanged)) {
              widget
                  .onChanged!(IZIDate.formatDate(value, format: 'yyyy/MM/dd'));
            }
          }
        },
      );
  ...
  Widget? getSuffixIcon() {
    ...
    if (widget.type == IZIInputType.PRICE) {
      ...
      //             "\$",
      //             style:
      //                 TextStyle(fontSize: IZIDimensions.FONT_SIZE_SPAN_SMALL),
      //           ),
      //         ),
      //       ),
      //     );
  ...
  Widget build(BuildContext context) {
    ...
    if (!IZIValidate.nullOrEmpty(widget.errorText) &&
        IZIValidate.nullOrEmpty(_errorText)) {
        ...
        children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    constraints: BoxConstraints(
                      maxHeight: IZIDimensions.ONE_UNIT_SIZE * 50,
                    ),
                    ...
                    decoration: BoxDecoration(
                      color: ColorResources.WHITE,
                      boxShadow:
                          widget.isNotShadown! ? null : IZIOther().boxShadow,
              ...
              Flexible(
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    width: IZIInputType.INCREMENT == widget.type
                        ? widget.width ?? IZIDimensions.ONE_UNIT_SIZE * 90
                        : null,
                    ...
                    child: TextFormField(
                      ...
                      textAlign: IZIInputType.INCREMENT == widget.type
                          ? TextAlign.center
                      ...
                      onFieldSubmitted: (val) {
                        ...
                        if (!IZIValidate.nullOrEmpty(val) &&
                            IZIInputType.INCREMENT == widget.type) {
                          if (IZINumber.parseInt(val) < widget.min!) {
                            getController(widget.type).text =
                                IZINumber.parseInt(widget.min.toString())
                                    .toString();
                          }
                        }
                      },
                      ...
                      obscureText: widget.obscureText ??
                          widget.type == IZIInputType.PASSWORD && isVisible,
                      focusNode: focusNode,
                      inputFormatters: widget.inputFormatters,
                      style: widget.style,
                      cursorColor:
                          widget.cursorColor ?? ColorResources.PRIMARY_1,
                      textCapitalization:
                      ...
                      decoration: InputDecoration(
                        ...
                        prefixIcon: IZIValidate.nullOrEmpty(widget.prefixIcon)
                            ? null
                            : widget.type == IZIInputType.PRICE
                                ? SizedBox.shrink(
                                    child: Padding(
          ...
          if (!IZIValidate.nullOrEmpty(_errorText) && !widget.disbleError)
            Container(
              ...
              child: Text(
                IZIValidate.nullOrEmpty(_errorText.toString())
                    ? ""
                    : _errorText.toString(),
